...
## v0.8.0 - 2026-03-09
* **狀態**: Phase 8 完成 — 企業品牌整合與全局編號同步
* **更新內容**:
  * [UI] 品牌重塑：導入「諮力管理顧問有限公司」主視覺、標語、任務宣言。
  * [UI] 安全聲明：首頁新增 100% Localhost 隱私保護說明區塊。
  * [UI] 聯絡功能：新增資深顧問聯絡名片與直連郵件功能。
  * [問卷] 支援自訂文件編號前綴（例如：IS 變更為 ABC）。
  * [問卷] IS-001 實作動態「ISMS 涵蓋範圍」填寫。
  * [引擎] 全局編號同步：Word 與 Excel 內文中的所有 `IS-` 編號自動隨前綴替換。
  * [引擎] 打包優化：ZIP 內目錄名稱之空白字元自動替換為 `-`。
  * [修復] 移除問卷中關於風險值建議的硬編碼提示。
